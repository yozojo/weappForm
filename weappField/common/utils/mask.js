 ...
 * @return {String}
 */
export const unmaskText = (text, mask) => {
  const length = mask ? mask.replace(new RegExp(defaultDelimiters, 'g'), '').length : 1000;
  return text ? String(text).replace(new RegExp(defaultDelimiters, 'g'), '').slice(0, length) : text
}
